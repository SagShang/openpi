...
The command will log training progress to the console and save checkpoints to the `checkpoints` directory. TensorBoard event files are written to `checkpoints/<config>/<exp_name>/tensorboard`, so you can monitor training with:

```bash
uv run tensorboard --logdir checkpoints/pi05_libero/my_experiment/tensorboard
```

For maximally using the GPU memory, set `XLA_PYTHON_CLIENT_MEM_FRACTION=0.9` before running training -- this enables JAX to use up to 90% of the GPU memory (vs. the default of 75%).
